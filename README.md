...
- **资源蜜蜂**：463 种蜂名，单一真源（资源包）+ 生成器派生所有层：客户端
  显示层（tooltip/名牌/JEI 类型行）+ 服务端按 ID 的数据迁移（蜂笼/实体旧名
  自动转正）。服务端数据保持上游英文，与 JEI/配方永远一致。
...
蜂名类文本另有专门体系：`scripts/gen_pb_hanhua.py` 以资源包为唯一真源生成
双端 kubejs 脚本（客户端显示层 + 服务端按 ID 数据迁移），check.py 查漂移。
**服务端严禁语言注入**（会与 JEI/配方数据分裂，玩家查不到配方）。
