...
# 对数不再在那条曲线上；真正贵的只剩下面 MAX_FRAGMENT_PAIRS 管的子串路径。
# 因此放宽到 1600，而 MAX_FRAGMENT_PAIRS **一个字都不动**——放宽的是已经不花钱的
# 那一半，不是花钱的那一半。放宽的前提是补丁必须真的打上了：patch_vaultpatcher.py
# 全程 fail-closed，打不上就退出，所以不存在「没打补丁却用着大表」的组合。
MAX_DYNAMIC_PAIRS = 1600
