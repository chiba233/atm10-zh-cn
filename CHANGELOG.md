...
- LICENSE 加入作者署名（Copyright © 2026 星野夢華）
- 安装器：三平台、自动备份 / 恢复、options.txt 自动启用、可选拼音搜索、
  实例目录自动检测失败时支持手动输入路径
- 质量基建：踩过的雷转为 CI 硬检查（协议值 / 生成器漂移 / 旧译名残留 /
  服务端模块安全）；安装脚本在 macOS / Windows / Linux 三系统跑端到端测试

**已知限制**

- Sodium「粒子效果」里的**原版**粒子名无法汉化：sodium-extra 0.9.3 自身问题
